    ...
    <div class="admin-content">
        ...
        <form action="{{ route('admin.ukm.store') }}" enctype="multipart/form-data" method="POST"
            ...
            <div class="row mb-3">
                ...
                <div class="col-12 col-md-10">
                    <input type="text" class="form-control" value="{{ old('product') }}" id="product"
                        name="product">
                </div>
            </div>
            <div class="row mb-3">
            ...
            <div class="row mb-3">
                ...
                <div class="col-12 col-md-10">
                    <input type="tel" class="form-control" value="{{ old('whatsapp') }}" id="whatsapp"
                        name="whatsapp">
            ...
            <div class="row mb-3">
                <label for="catprogramalog" class="col-12 col-md-2 col-form-label">Asal Program</label>
                <div class="col-12 col-md-10">
            ...
            <div class="row mb-3 non-global">
                ...
                <div class="col-12 col-md-5 mb-2 mb-md-0">
                    <input type="time" class="form-control" value="{{ old('operational_hours') }}"
                        id="operational_hours" name="operational_hours">
                </div>
                <div class="col-12 col-md-5">
                    <input type="time" class="form-control" value="{{ old('operational_hours_end') }}"
                ...
                <div class="col-12 col-md-10">
                    <select type="text" class="form-control" multiple aria-label="multiple size 4 select example"
                        ...
                        @foreach ($categories as $category)
                            <option
                                @if (old('categories[]')) {{ in_array($category->title, old('categories[]')) ? 'selected' : '' }} @endif
            ...
            <tbody>
                @foreach ($ukms as $ukm)
                    <tr>
                        ...
                        <td>{{ $ukm->program_id ? $ukm->program->title : '-' }}</td>
                        <td class="categories">
                            @foreach ($ukm->categories as $item)
                                <span>{{ $item->title }}</span>
                            @endforeach
                        </td>
                        <td>{{ $ukm->address . ', ' . ucwords(strtolower($ukm->city_name)) . ', ' . ucwords(strtolower($ukm->state_name)) }}
                        </td>
                        <td>{{ views($ukm)->period(\CyrildeWit\EloquentViewable\Support\Period::since('2021-11-18'))->unique()->count() }}
                        </td>
                        <td>{{ $ukm->whatsapp }}</td>
                        <td>{{ $ukm->instagram }}</td>
                        <td>{!! !str_contains(strtolower($ukm->catalog->title), 'global') ? $ukm->achievement : '-' !!}</td>
                        <td>{{ !str_contains(strtolower($ukm->catalog->title), 'global') ? $ukm->operational_hours . '-' . $ukm->operational_hours_end : '-' }}
                        </td>
                        <td>{!! str_contains(strtolower($ukm->catalog->title), 'global') ? $ukm->permission : '-' !!}</td>
                        <td>{{ str_contains(strtolower($ukm->catalog->title), 'global') ? $ukm->capacity : '-' }}
                        </td>
